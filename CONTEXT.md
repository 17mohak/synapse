...
| m1 | (this commit) | 2026-06-01 | Extend `client.ts` types: new `TokenTrajectory` interface; add `trajectories` and `entropy` to `AnalyzeResponse`. Pure type plumbing. **No `store.ts` change** (the store already holds the full `AnalyzeResponse`, so the new fields reach components via `result` automatically). Interaction-state fields deferred to M7/M9. No UI reads, no visual change. | Revert the commit; types drop the new fields. Nothing reads them yet; old grid and app unaffected. |
