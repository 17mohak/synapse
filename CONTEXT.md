...
| rc1 | (this commit) | 2026-06-02 | **Recompose 1 — retire redundant views.** Vision B recompose ("doubt has a shape, and you can see it"): the page must read as one object, not a dashboard. Removed the next-token top-10 list (the climax stated as a spreadsheet, *above* the climax — the biggest spoiler) and the old logit-lens grid (a redundant third view of one prediction) from `App.tsx`; The Race is now the single centerpiece. Components remain in the repo, unmounted. Applied: dataviz-data-storytelling ("don't hand back a dashboard") + impeccable/distill (one primary goal). | Revert; re-mount `NextToken` + `LogitLensView` in `App.tsx`. ClimbView unaffected. |
