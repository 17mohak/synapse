...
| m2 | (this commit) | 2026-06-01 | Add `ClimbView` (axes-only scaffold for The Race), mounted **below** the live grid in `App.tsx`. X = layers 0–11, Y = real probabilities on an adaptive linear domain (0 → global-peak prob × 1.2, capped 100%). First D3 work; first visual checkpoint. Old grid untouched and fully functional. **Open finding (decide at M3/M4):** the global-peak domain is set by spurious early-layer / echo spikes (`' the'@L1=0.83`, `' lazy'@L0=0.999`) not the race, so it saturates ~100% and will crush the late-layer competition. Candidate fix: adapt Y to a late-layer window (L9–11 max: Eiffel 0.38, fox 0.63, varied) once trajectories are drawn. | Revert the commit; `ClimbView` unmounts, `App.tsx` import removed. Grid and app unaffected. |
